...
func GetAReply(reply_id int) (*model.Reply, error) {
	...
	query := "SELECT r.reply_id, r.content, r.postedAt , r.postedBy_id, r.postedTo_id, r.edited AS editedAt, r.deleted AS deleted_reply, u.username, u.deleted, r.IsToPost FROM replies r JOIN users u ON r.postedBy_id = u.user_id WHERE r.reply_id = ?"
